class RentaltemplatePolicy < ApplicationPolicy
  class Scope < Scope
    ...
    def resolve
      ...
      scope.where(user: user).or(scope.where(user_id: User.where(admin: true).first.id).where(public: true)) # If users can only see their records
